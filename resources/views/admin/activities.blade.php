...
<body>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="status-cell">
                        <select class="form-select status-dropdown" data-id="{{ $activity->id }}" style="cursor: pointer;">
                            <option value="0" {{ $activity->status == 0 ? 'selected' : '' }}>Pending</option>
                            <option value="1" {{ $activity->status == 1 ? 'selected' : '' }}>Approved</option>
                            <option value="2" {{ $activity->status == 2 ? 'selected' : '' }}>Rejected</option>
                        </select>
                    </td>
                    <td class="action-buttons">
                        @if ($activity->status == 1)
                        <a href="{{ route('activities.manage', $activity->id) }}" class="btn btn-dark btn-sm">Manage</a>
                        @endif
            ...
            <div class="modal-content">
                <form id="createActivityForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="location" class="form-label">Provincial</label>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            form.addEventListener("submit", function(e) {
                ...
                fetch("/activities/store", {
                    ...
                    <td>${activity.date}</td>
                    <td class="status-cell">
                        <select class="form-select status-dropdown" data-id="${activity.id}">
                            <option value="0" ${activity.status == 0 ? 'selected' : ''}>Pending</option>
                            <option value="1" ${activity.status == 1 ? 'selected' : ''}>Approved</option>
                            <option value="2" ${activity.status == 2 ? 'selected' : ''}>Rejected</option>
                        </select>
        ...
        // Update Status
        document.querySelector("#activityTable").addEventListener("change", function(event) {
            if (event.target.classList.contains("status-dropdown")) {
                const select = event.target;
                const activityId = select.getAttribute("data-id");
        ...
        document.querySelector("#activityTable").addEventListener('click', function(event) {
            if (event.target.classList.contains('btn-danger')) {
                ...
                const activityId = row.querySelector('td:first-child').textContent;
